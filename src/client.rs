...
impl Client {
  ...
  pub fn build_contract(&self, path: Either<&str, Vec<u8>>, address: &str) -> Contract<web3::transports::Http> {
  ...
  async fn get_erc20token_balance<'a>(&self, token: &'a str, holder: &'a str) -> f64 {
    ...
    let result = Self::try_fetch_balance_of(token_contract.clone(), holder).await.unwrap();
  ...
  pub async fn get_ftm_gton_liq(&self) -> f64 {
    ...
    let f_token_balance_of_fg_spi_lp = self.get_erc20token_balance(
      f_token.0,
      fg_spi_lp.0
    ).await;
  ...
  pub async fn get_gc_pol(&'static self) -> f64 {
    let (
      ...
      usdc_gton_liq,
      usdc_gton_lp,
    ): (f64, f64, f64, f64, f64) = tokio::join!(
  ...
  pub async fn get_pw_model_with_pol_mln(
    ...
    f64::max(
      gc_floor,
      gc_bias + (gc_max_p * _pol / gc_max_l)
    )
  }
  ...
  pub async fn get_gc_pw_current_peg_ftm(&'static self) -> f64 {
    let (gc_pol, wftm_price) = tokio::join!(
      self.get_gc_pol(),
      self.get_wftm_price()
    );
...
mod tests {
  ...
  async fn test_get_wftm_price() {
    let client = Client::new(Props { node_rpc: String::from("https://rpcapi-tracing.fantom.network") }).await;
    println!("get_wftm_price: {:?}", client.get_wftm_price().await);
  }

  #[tokio::test]
  async fn test_get_wftm_gton_gc_pool_lp() {
    let client = Box::leak(Box::new(Client::new(Props { node_rpc: String::from("https://rpcapi-tracing.fantom.network") }).await));
    println!("get_wftm_gton_gc_pool_lp: {:?}", client.get_wftm_gton_gc_pool_lp().await);
  }

  #[tokio::test]
  async fn test_get_usdc_gton_gc_pool_lp() {
    let client = Box::leak(Box::new(Client::new(Props { node_rpc: String::from("https://rpcapi-tracing.fantom.network") }).await));
    println!("get_usdc_gton_gc_pool_lp: {:?}", client.get_usdc_gton_gc_pool_lp().await);
  }
  ...
  async fn test_get_gton_usdc_price() {
    let client = Box::leak(Box::new(Client::new(Props { node_rpc: String::from("https://rpcapi-tracing.fantom.network") }).await));
    println!("get_gton_usdc_price: {:?}", client.get_gton_usdc_price().await);
  }
  
  #[tokio::test]
  async fn test_get_gton_wftm_price() {
    let client = Box::leak(Box::new(Client::new(Props { node_rpc: String::from("https://rpcapi-tracing.fantom.network") }).await));
    println!("get_gton_wftm_price: {:?}", client.get_gton_wftm_price().await);
  }

  #[tokio::test]
  async fn test_get_gc_pw_current_peg_usd() {
    let client = Box::leak(Box::new(Client::new(Props { node_rpc: String::from("https://rpcapi-tracing.fantom.network") }).await));
    println!("get_gc_pw_current_peg_usd: {:?}", client.get_gc_pw_current_peg_usd().await);
  }

  #[tokio::test]
  async fn test_get_gc_pw_current_peg_ftm() {
    let client = Box::leak(Box::new(Client::new(Props { node_rpc: String::from("https://rpcapi-tracing.fantom.network") }).await));
    println!("get_gc_pw_current_peg_ftm: {:?}", client.get_gc_pw_current_peg_ftm().await);
  }
}